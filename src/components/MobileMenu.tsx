...
export function MobileMenu({ user }: { user?: SidebarUser }) {
  ...
  const drawer = (
    <div className={`fixed inset-0 z-[60] md:hidden ${open ? "visible" : "invisible"}`} aria-hidden={!open}>
      ...
      />
      <aside
        className={`absolute inset-y-0 left-0 flex w-72 max-w-[82%] flex-col bg-white shadow-2xl transition-transform duration-300 ${
      ...
      >
        <button
          ...
          className="absolute right-3 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-slate-700"
        >
